...
async function startTUI() {
  ...
  function showContent(data, englishTitle) {
    ...
    if (data.episodes && data.episodes.length > 0) {
      ...
      data.episodes.forEach((e, i) => {
        body.add(new TextRenderable(renderer, {
          id: 'c-ep-' + i,
          content: '   ' + (i + 1) + '. ' + e.title.substring(0, 60),
          fg: '#AAAAAA',
        }));
      });
    }
  ...
  function showEpisodeList(episodes) {
    ...
    const options = episodes.map((e, i) => ({
      name: (i + 1) + '. ' + e.title.substring(0, 70),
      description: '',
      value: e,
    }));
  ...
  function goBack() {
    ...
    switch (prev) {
      ...
      case Views.RESULTS: showResults(cachedData ? [cachedItem] : [], ''); break;
      case Views.CONTENT: if (cachedData) showContent(cachedData, ''); break;
      case Views.EPISODES: if (cachedData) showEpisodeList(cachedData.episodes || []); break;
