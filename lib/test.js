...
// Pattern 7: Website suffix stripped
result = parseTitle('مسلسل The Testaments الموسم الاول الحلقة 10 مترجمة   | ايجي ديد');
assert('Website suffix: strips | ايجي ديد', result.english === 'The Testaments S01E10');

// Pattern 8: Arabic-only show name with الجزء (part)
result = parseTitle('مسلسل احتمال حب الجزء الاول الحلقة 15 مدبلجة');
assert('Arabic الجزء: show name preserved', result.show === 'احتمال حب');
assert('Arabic الجزء: season 1', result.season === '1');
assert('Arabic الجزء: episode 15', result.episode === '15');
assert('Arabic الجزء: formatted', result.english === 'احتمال حب S01E15');

// Pattern 9: برنامج prefix (TV show)
result = parseTitle('برنامج Baylen Out Loud الموسم الثالث الحلقة 7 مترجمة');
assert('برنامج prefix: extracts show name', result.show === 'Baylen Out Loud');
assert('برنامج prefix: season 3', result.season === '3');
assert('برنامج prefix: episode 7', result.episode === '7');

// Pattern 10: Season with season 5 (الخامس)
result = parseTitle('مسلسل The Boys الموسم الخامس الحلقة 8 مترجمة');
assert('Season الخامس: show name', result.show === 'The Boys');
assert('Season الخامس: season 5', result.season === '5');

// Pattern 11: Anime with episode only
result = parseTitle('انمي مذكرة الموت الحلقة 25 مترجمة');
assert('Anime Arabic: show name', result.show === 'مذكرة الموت');
assert('Anime Arabic: episode 25', result.episode === '25');
assert('Anime Arabic: formatted', result.english === 'مذكرة الموت E25');

// Pattern 12: Season page with season 3
result = parseTitle('مسلسل Silo الموسم الثالث مترجم كامل');
assert('Season page s3: show name', result.show === 'Silo');
assert('Season page s3: season 3', result.season === '3');

// Pattern 13: Movie مشاهدة prefix only (no فيلم)
result = parseTitle('مشاهدة فيلم Man of War 2026 مترجم');
assert('Movie مشاهدة فيلم: name', result.show === 'Man of War');
assert('Movie مشاهدة فيلم: year 2026', result.year === '2026');

// Pattern 14: Fallback - clean Arabic structural words
result = parseTitle('مسلسل The Greatest Rivalry India vs Pakistan 2025 مترجم كامل');
assert('Fallback: mostly English preserved', result.english.includes('The Greatest Rivalry India vs Pakistan'));
